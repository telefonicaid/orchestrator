...
LOGGING = {
    ...
    'formatters': {
        'standard': {
            ...
            'format' : 'time=%(asctime)s.%(msecs)03dZ | lvl=%(levelname)s | corr=%(correlator)s | trans=%(transaction)s | from=%(from)s | srv=%(service)s | subsrv=/%(subservice)s | comp=Orchestrator | op=%(name)s:%(funcName)s() | msg=%(message)s',
