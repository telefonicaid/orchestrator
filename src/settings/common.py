...
LOGGING = {
    ...
    'formatters': {
        'standard': {
            ...
            'format' : 'time=%(asctime)s.%(msecs)03dZ | lvl=%(levelname)s | corr=%(correlator)s | trans=%(transaction)s | srv=%(service)s | subsrv=/%(subservice)s | component=Orchestrator | op=%(name)s:%(funcName)s() | msg=%(message)s',
